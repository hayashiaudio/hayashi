...
describe('compileDspToNative', () => {
  it('compiles a simple oscillator to VST3', async () => {
    const result = await compileDspToNative(SIMPLE_OSC, 'TestOsc', `test-id-${Date.now()}`, 'v1', 'vst3');
    expect(result.fromCache).toBe(false);
    expect(result.downloadUrl).toContain('plugin.vst3');
  });

  it('returns cached result on second call', async () => {
    const pluginId = `cache-test-${Date.now()}`;
    const first = await compileDspToNative(SIMPLE_OSC, 'TestOsc', pluginId, 'v1', 'vst3');
    expect(first.fromCache).toBe(false);
    const second = await compileDspToNative(SIMPLE_OSC, 'TestOsc', pluginId, 'v1', 'vst3');
